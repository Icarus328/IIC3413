...
public:
  ...
  // page it's empty when all dirs are marked as -2
  bool is_page_empty() const;

  // normally we only delete a record logically (mark as deleted)
  void delete_record_logically(int32_t dir_pos, TxID tx_id);

  // used only for abort transaction and full vacuum
  ...
  void reset_update(int32_t dir_pos);

  void reset_first(int32_t dir_pos);
...
private:
  ...
  // Same as get_record(), but without locking edit_mutex
  // Only used by vacuum()
  Record _get_record(int32_t dir_pos) const;
