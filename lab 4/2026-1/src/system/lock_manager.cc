...
bool LockManager::try_acquire_lock(TxID tx_id, uint64_t table_id, RID first_rid) {
  ...
  while (true) {
    ...
    if (time_elapsed > MAX_TIME) {
      transaction_mgr.abort_transaction(tx_id);
      throw TransactionAbortException("Transaction " + std::to_string(tx_id) + " aborted due to lock timeout");
    }
    {
      ...
      if (it == lock_table.end()) {
        ...
        transaction_mgr.add_to_write_set(table_id, key.first_rid, tx_id);
