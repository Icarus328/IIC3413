...
std::string Session::execute_query(const std::string& query, TxID tx_id, bool commit_at_end) {
  try {
    ...
    throw; // tx already aborted internally
  } catch (...) {
    if (commit_at_end) {
      transaction_mgr.abort_transaction(tx_id);
    }
    throw; // this is caught at process_query()
  }
}
...
std::string Session::run_query() {
  if (state == 0) {
    ...
    if (my_compare(query, "BEGIN TRANSACTION")) {
      ...
      if (my_compare(query, "BEGIN TRANSACTION SNAPSHOT ISOLATION")) {
        tx_id = transaction_mgr.start_transaction(IsolationLevel::SNAPSHOT_ISOLATION);
      } else {
        tx_id = transaction_mgr.start_transaction(IsolationLevel::READ_COMMITTED);
      }
