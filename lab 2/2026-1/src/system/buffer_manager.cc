...
void BufferManager::delete_file(FileId file_id) {
  {
...
void BufferManager::delete_page(FileId file_id, int64_t page_number) {
  {
    std::lock_guard<std::mutex> lck(pages_mutex);
...
Page& BufferManager::get_page(FileId file_id, int64_t page_number) {
  ...
  if (it == page_map.end()) {
    ...
    if (page.page_id.file_id.fd != FileId::UNASSIGNED) {
