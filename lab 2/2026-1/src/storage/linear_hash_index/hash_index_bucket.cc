...
void BucketPage::delete_record(RID rid) {
    //Obtenemos el numero de registros en la pagina
    int64_t num_tuples = get_tuple_count();

    //Recorremos las entradas de la pagina
    for (int64_t i = 0; i < num_tuples; ++i) {
        //Registro en i
        HashIndexRecord current_record = get_record(i);

        //Comparamos RID
        if (current_record.rid == rid) {
            //Si lo encontramos, lo borramos
            //Movemos la ultima entrada a esta posicion
            if (i < num_tuples - 1) {
                HashIndexRecord last_record = get_record(num_tuples - 1);
                set_record(i, last_record); //Sobreescribimos el registro i con el ultimo
            }
            
            //Actualizamos el contador de tuplas
            num_tuples--;
            set_tuple_count(num_tuples);

            //Como movimos el ultimo registro a i, revisamos de nuevo i en la siguiente iteración
            //Por si el registro que movimos también debe ser eliminado
            i--; 
        }
    }
}