  ...
  if (Filesystem::exists(db_folder)) {
    if (!Filesystem::is_directory(db_folder)) {
      throw std::invalid_argument(
          "Cannot create database directory: \"" + db_folder + "\", a file with that name already exists."
      );
    }
  } else {
...
void FileManager::delete_file(FileId file_id) {
  std::string file_path;
  for (auto it = filename2file_id.begin(); it != filename2file_id.end(); it++) {
    if (it->second.fd == file_id.fd) {
      file_path = it->first;
      filename2file_id.erase(it);
      break;
    }
  }
  if (!file_path.empty()) {
...
void FileManager::truncate_file(FileId file_id, int64_t new_page_count) {
  auto fd = file_id.fd;
  auto write_res = ftruncate(fd, new_page_count * Page::SIZE);
  if (write_res == -1) {
